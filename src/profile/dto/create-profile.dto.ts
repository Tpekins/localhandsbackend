...
export class CreateProfileDto {
  @ApiProperty({ example: 1, description: 'ID of the user this profile belongs to' })
  @IsInt({ message: 'User ID must be an integer and is required.' })
  userId: number;

  @ApiProperty({ example: 'I am a professional service provider', description: 'User biography', required: false })
  @IsOptional()
  @IsString({ message: 'Bio must be a string if provided.' })
  @Length(0, 500, { message: 'Bio must not exceed 500 characters.' })
  bio?: string;

  @ApiProperty({ example: '+1234567890', description: 'Mobile money account number', required: false })
  @IsOptional()
  ...
  @ApiProperty({ example: 'https://example.com/id.jpg', description: 'URL to the national ID document', required: false })
  @IsOptional()
